...
contenttype=$(curl -o /dev/null --head -w "%{content_type}\n" -X GET -u admin:Admin123 --connect-timeout 5 --max-time 20 http://localhost:8099/openmrs/ws/rest/v1/visittype 2>/dev/null | cut -d ";" -f 1)
...
test_dir=$(mktemp -d --tmpdir -t analytics_tests__XXXXXX)
...
echo "java -cp $(echo batch/target/fhir-batch-etl-bundled-*.jar | cut -d " " -f 1) org.openmrs.analytics.FhirEtl --openmrsServerUrl=http://localhost:8099/openmrs --openmrsUserName=admin --openmrsPassword=Admin123  --searchList=Patient,Encounter,Observation --batchSize=20 --targetParallelism=20 --fileParquetPath='${test_dir}'/"
java -cp $(echo batch/target/fhir-batch-etl-bundled-*.jar | cut -d " " -f 1) org.openmrs.analytics.FhirEtl --openmrsServerUrl=http://localhost:8099/openmrs --openmrsUserName=admin --openmrsPassword=Admin123  --searchList=Patient,Encounter,Observation --batchSize=20 --targetParallelism=20 --fileParquetPath="${test_dir}"/
...
curl -L -X GET -u admin:Admin123 --connect-timeout 5 --max-time 20 http://localhost:8099/openmrs/ws/fhir2/R4/Patient/ 2>/dev/null >> ./omrs/patients.json
total_test_patients=$(jq '.total' ./omrs/patients.json)
echo "${print_prefix} Total openmrs test patients ---> ${total_test_patients}"
curl -L -X GET -u admin:Admin123 --connect-timeout 5 --max-time 20 http://localhost:8099/openmrs/ws/fhir2/R4/Encounter/ 2>/dev/null >> ./omrs/encounters.json
total_test_encounters=$(jq '.total' ./omrs/encounters.json)
echo "${print_prefix} Total openmrs test encounters ---> ${total_test_encounters}"
curl -L -X GET -u admin:Admin123 --connect-timeout 5 --max-time 20 http://localhost:8099/openmrs/ws/fhir2/R4/Observation/ 2>/dev/null >> ./omrs/obs.json
...
fhir_server_status_code=$(curl -o /dev/null --head -w "%{http_code}" -L -X GET -u hapi:hapi --connect-timeout 5 --max-time 20 http://localhost:8098/fhir/Observation 2>/dev/null)
...
curl -L -X GET -u hapi:hapi --connect-timeout 5 --max-time 20 http://localhost:8098/fhir/Patient/?_summary=count 2>/dev/null >> ./fhir/patients.json
