...
public class FhirSearchUtil {
	...
	Bundle searchForResource(String baseSearchUrl, int count, int first, String jsessionId, boolean parse,
		...
		try {
			...
			HttpUriRequest request = RequestBuilder.get().setUri(uriBuilder.build())
			        ...
			        .addHeader("Accept", "application/fhir+json").addHeader("Cookie", "JSESSIONID=" + jsessionId)
			        .addParameter("_count", String.valueOf(count)).addParameter("_getpagesoffset", String.valueOf(first))
			        .addParameter("_summary", "data").build();
			...
			if (parse) {
				...
				log.info(
				    String.format("Fetched %d resources out of a total of %d", bundle.getEntry().size(), bundle.getTotal()));
				return bundle;
			} else {
				return null;
			}
	...
	public void uploadBundleToCloud(Bundle bundle) {
		...
		for (BundleEntryComponent entry : bundle.getEntry()) {
			...
			fhirStoreUtil.uploadResourceToCloud(resource.getResourceType().name(), resource.getIdElement().getIdPart(),
