...
public class ReadJsonFilesFn extends FetchSearchPageFn<FileIO.ReadableFile> {
	 ...
	 * references are resolved properly, i.e., if the referenced resource is in the Bundle, it is found.
	 * However, the references are kept as original URNs. With this function we are trying to simulate
	 * the logic of uploading the Bundle to a FHIR server and then downloading those resources. The
	 * logic implemented here is based on how HAPI parser resolves the same references within a Bundle;
	 * see {@code ca.uhn.fhir.parser.ParserState.stitchBundleCrossReferences}.
	 *
	 * @param bundle the bundle whose references are updated.
	 */
	private void updateResolvedRefIds(Bundle bundle) {
		for (BundleEntryComponent entry : bundle.getEntry()) {
			List<IBaseReference> refs = fhirContext.newTerser().getAllPopulatedChildElementsOfType(entry.getResource(),
			...
			for (IBaseReference ref : refs) {
				...
				if (resource != null) {
					if (ref.getReferenceElement() == null || ref.getReferenceElement().getIdPart() == null
						...
						log.debug(String.format("Updating URN reference %s to the resolved resource ID %s",
