...
public class StatusServer {
	...
	static class EventTimeHandler implements HttpHandler {
		...
		synchronized public void setVar(String name, String value) {
			statusVars.put(name, value);
		}
		
		synchronized private String getVar(String name) {
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			if (var.isEmpty() || !var.startsWith("/")) {
				StringBuilder builder = new StringBuilder(
				        String.format("The path format should be %s/VAR_NAME for VAR_NAME in:\n", PATH_PREFIX));
