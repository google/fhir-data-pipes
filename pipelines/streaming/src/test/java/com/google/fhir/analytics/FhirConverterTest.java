...
public class FhirConverterTest extends CamelTestSupport {
  ...
  @Test
  public void shouldFetchFhirResourceAndUploadToFhirStore() throws IOException, ProfileException {
    ...
    Mockito.when(fetchUtil.fetchFhirResource(Mockito.anyString())).thenReturn(resource);
    ...
    Mockito.verify(fetchUtil).fetchFhirResource(Mockito.anyString());
  ...
  public void shouldGetUuidFromParent() throws SQLException {
    ...
    Mockito.verify(fetchUtil).fetchFhirResource("/Patient/UUID");
    Mockito.verify(uuidUtil).getUuid("person", "person_id", "1");
  }

  @Test
  public void shouldFetchFhirResourceAndOutputParquet() throws IOException, ProfileException {
    Map<String, String> messageBody = DebeziumTestUtil.genExpectedBody();
    Map<String, Object> messageHeaders =
        DebeziumTestUtil.genExpectedHeaders(Operation.UPDATE, "encounter");
    resource = new Encounter();
  ...
  public void shouldIgnoreDeleteEvent() {
    ...
    Mockito.verify(fetchUtil, Mockito.times(0)).fetchFhirResource(Mockito.anyString());
  }
  ...
  public void shouldGenerateFhirResourcesForTablesThatHaveBeenMappedInConfig() {
    ...
    Mockito.verify(fetchUtil, Mockito.times(tables.length)).fetchFhirResource(Mockito.anyString());
  }
