...
def create_max_obs_sql_query(
    base_patient_url: str, start_date: str, end_date: str, code_list: List[str]
) -> str:
    ...
    sql_query = (
        ...
        + 'AND observation.effective.dateTime > "'
        + start_date
        + '" '
        + 'AND observation.effective.dateTime < "'
        + end_date
...
def get_max_obs(
    ...
    obs_filtered = (
        observation.select("subject.patientId", "effective.dateTime", "code", "value")
        .filter(observation.effective.dateTime > start_date)
        .filter(observation.effective.dateTime < end_date)
    )
    obs_codes = obs_filtered.select(
        obs_filtered.patientId,
        obs_filtered.dateTime,
        obs_filtered.value,
        explode(obs_filtered.code.coding).alias("coding"),
    )
    obs_target_codes = obs_codes.filter(obs_codes.coding.code.isin(code_list))
    grouped_obs = obs_target_codes.groupBy(["patientId", "coding.code"]).agg(
        {"dateTime": "max", "value.quantity": "max", "*": "count"}
    )
    patient_name = patient.select(
        patient.id, patient.name[0].family.alias("family")
    ).withColumn("actual_id", regexp_replace("id", base_patient_url, ""))
    return patient_name.join(
        grouped_obs, patient_name.actual_id == grouped_obs.patientId
    )
