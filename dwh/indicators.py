...
def create_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--output_csv", help="The output CSV file", required=True, type=str
    )
...
def find_date_range(args: argparse.Namespace) -> Tuple[str, str, str, str, str]:
    ...
    return (
        start_date_str,
        end_date_str,
        previous_period_start_str,
        semiannual_start_str,
        quarterly_start_str,
    )
...
if __name__ == "__main__":
    ...
    (start_date, end_date, prev_start, semiannual_start_str, quarterly_start_str) = (
        find_date_range(args)
    )
    ...
    ).include_obs_values_in_time_range(_VL_CODE, min_time=start_date, max_time=end_date)
    ...
    semi_annual_query = query_lib.patient_query_factory(
        query_lib.Runner.SPARK, args.src_dir, _CODE_SYSTEM
    ).include_all_other_codes(min_time=semiannual_start_str, max_time=end_date)
    # Prev Month query
    prev_month_query = query_lib.patient_query_factory(
        query_lib.Runner.SPARK, args.src_dir, _CODE_SYSTEM
    ).include_all_other_codes(min_time=prev_start, max_time=end_date)
    # Quarterlly query
    quarterly_query = query_lib.patient_query_factory(
        query_lib.Runner.SPARK, args.src_dir, _CODE_SYSTEM
    ).include_all_other_codes(min_time=quarterly_start_str, max_time=end_date)
    ...
    TX_NEW_df = indicator_lib.calc_TX_NEW(
        current_month_df,
        ARV_plan=_ARV_PLAN,
        start_drug=[_START_DRUGS],
        end_date_str=end_date,
    )

    TB_STAT_df = indicator_lib.calc_TB_STAT(
        current_month_df,
        TB_TX_plan=_TX_TB_PLAN,
        ARV_plan=_ARV_PLAN,
        TB_plan_answer=[_START_DRUGS],
        end_date_str=end_date,
    )
    ...
    TX_TB_df = indicator_lib.calc_TX_TB(
        annual_df,
        TX_TB_plan=_TX_TB_PLAN,
        ARV_plan=_ARV_PLAN,
        TB_screening=_TB_screening,
        YES_CODE=_YES_CODE,
