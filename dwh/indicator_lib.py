...
def calc_TX_PVLS(patient_agg_obs: pandas.DataFrame, VL_code: str,
  ...
  temp_df[['vl_max_date', 'latest_vl_value']] = temp_df.max_date_value.str.split('_SeP_', expand=True)
...
def calc_TX_NEW(patient_agg_obs:  pandas.DataFrame, ARV_plan: str, start_drug: List,
  ...
  # separate value from date
  temp_df[['ARV_max_date', 'latest_ARV_plan']] = temp_df.max_date_value_code.str.split('_SeP_', expand=True)
  # Note the above copy is used to avoid setting a new column on a slice next:
  ...
  temp_df_exp = temp_df_exp.groupby(['TX_NEW', 'buckets'], as_index=False) \
    .count()[['TX_NEW', 'buckets', 'patientId']] \
    .rename(columns={'patientId': 'TX_NEW_count'})
  # calculate ratio
  temp_df_exp['TX_NEW_ratio'] = temp_df_exp['TX_NEW_count'] / temp_df['TX_NEW'].count()
  return temp_df_exp