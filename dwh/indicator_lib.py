...
def calc_TX_PVLS(patient_agg_obs: pandas.DataFrame, VL_code: str,
  ...
  agg_buckets_udf = F.UserDefinedFunction(
      lambda a, g: _agg_buckets(a, g, end_date),
  ...
  VL_agg_P = VL_df.select(
      VL_df.sup_VL,
      F.explode(VL_df._agg_buckets).alias('agg_bucket')).groupBy(
...
# TODO remove this function once we verify that that pandas.DataFrame based
#   approach scales.
#def calc_TX_PVLS_spark(patient_agg_obs: DataFrame, VL_code: str, failure_threshold: int,
