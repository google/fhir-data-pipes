...
def calc_TX_CURR(patient_agg_obs:  pd.DataFrame, ARV_plan: str,
  ...
  Args:
    patient_agg_obs: A DataFrame generated by `patient_query.find_patient_aggregates()`.
    ARV_plan: The concept question code for ANTIRETROVIRAL PLAN
    start_drug: The concept answer codes for START DRUG, CONTINUE REGIMEN, REFILLED
    end_date_str: The string representation of the last date as 'YYYY-MM-DD'.
  Returns:
    The aggregated DataFrame with age/gender buckets.
  """
  ...
  if end_date_str:
    end_date = parse_date(end_date_str)
  temp_df = patient_agg_obs[(patient_agg_obs['code'] == ARV_plan)].copy()
  temp_df['TX_CURR'] = (temp_df['last_value_code'].isin(start_drug))
  temp_df = _gen_counts_and_ratio(temp_df, end_date, 'TX_CURR')
  return temp_df

def calc_TB_STAT(patient_agg_obs:  pd.DataFrame, TB_TX_plan: str,
                 ARV_plan: str,TB_test_status: str,start_drug: List[str],
                 yes_code, end_date_str: str = None) -> pd.DataFrame:
  ...
  Args:
    ...
    ARV_plan: The concept question code for ANTIRETROVIRAL PLAN
    TB_test_status: The concept question code for TB test done (Yes /No)
    start_drug: The concept answer codes for START DRUG
  ...
  if end_date_str:
    end_date = parse_date(end_date_str)
  # Check for TB TREATMENT PLAN (if START it means diagnosis was done)
  tb_tx_df = patient_agg_obs[(patient_agg_obs['code'] == TB_TX_plan)].copy()
  tb_tx_df['TB_TX'] = (tb_tx_df['last_value_code'].isin(start_drug))

  # check if TB was diagnosed
  tb_test_df = patient_agg_obs[(patient_agg_obs['code'] == TB_test_status)].copy()
  tb_test_df['TB_test'] = (tb_test_df['last_value_code'] == yes_code)
  ...
  # TODO: Need a simpler way of doing multi-code evaluation instead of the above
  # join the 3 DF
  temp_df=tb_tx_df.merge(tb_test_df[['patientId', 'TB_test']],
                         on='patientId').merge(art_tx_df[['patientId', 'HIV_positive']],
                                               on='patientId')
  # evaluate
  temp_df['TB_STAT'] = ((temp_df['HIV_positive'] == True) &
                        ((temp_df['TB_test'] == True) | (temp_df['TB_TX'] == True)))
