...
def calc_TX_PVLS(patient_agg_obs: pandas.DataFrame, VL_code: str,
  ...
  # separate value from date
  temp_df[['vl_max_date', 'latest_vl_value']] = temp_df.max_date_value.str.split(query_lib.DATE_VALUE_SEPARATOR,
                                                                                       expand=True)
  temp_df['latest_vl_value'] = temp_df.loc[:, 'latest_vl_value'].astype(float)

  temp_df['sup_VL'] = (temp_df.loc[:, 'latest_vl_value'] < failure_threshold)
  ...
  temp_df_exp = temp_df_exp.groupby(['sup_VL', 'buckets'], as_index=False)\
      .count().loc[:, ['sup_VL', 'buckets', 'patientId']]\
      .rename(columns={'patientId': 'sup_VL_count'})
  # calculate ratio
  num_patients = len(temp_df.index)
  temp_df_exp['sup_VL_ratio'] = temp_df_exp.loc[:, 'sup_VL_count']/num_patients
  return temp_df_exp


def calc_TX_NEW(patient_agg_obs:  pandas.DataFrame, ARV_plan: str, start_drug: List,
  ...
  # Note the above copy is used to avoid setting a new column on a slice next:
  # separate value from date
  temp_df[['ARV_max_date', 'latest_ARV_plan']] = temp_df.max_date_value_code.str.split(query_lib.DATE_VALUE_SEPARATOR,
                                                                                  expand=True)
  temp_df['TX_NEW'] = (temp_df.loc[:, 'latest_ARV_plan'].isin(start_drug))
  temp_df['buckets'] = temp_df.apply(
    lambda x: _agg_buckets(x.birthDate, x.gender, end_date), axis=1)
  temp_df_exp = temp_df.explode('buckets')
  temp_df_exp = temp_df_exp.groupby(['TX_NEW', 'buckets'], as_index=False) \
    .count().loc[:, ['TX_NEW', 'buckets', 'patientId']] \
    .rename(columns={'patientId': 'TX_NEW_count'})
  # calculate ratio
  num_patients = len(temp_df.index)
  temp_df_exp['TX_NEW_ratio'] = temp_df_exp.loc[:, 'TX_NEW_count'] / num_patients
  return temp_df_exp