...
def calc_TX_PVLS(patient_agg_obs: pd.DataFrame, VL_code: str,
  ...
  Args:
    ...
    end_date_str: The string representation of the last date as 'YYYY-MM-DD'.
  Returns:
...
def calc_TB_ART(patient_agg_obs:  pd.DataFrame, TB_TX_plan: str,ARV_plan: str,
                TB_plan_answer: List[str],ART_plan_answer: List[str],
                 end_date_str: str = None) -> pd.DataFrame:
...
def calc_TB_PREV(patient_agg_obs:  pd.DataFrame, TB_PREV_plan: str,ARV_plan: str,
  ...
  TB_PREV indicator counts of ART patients who started on a standard course of
...
def calc_TX_TB(patient_agg_obs:  pd.DataFrame, TB_TX_plan: str,ARV_plan: str,
                TB_plan_answer: List[str],ART_plan_answer: List[str],
  ...
  temp_df=tb_tx_df.merge(art_tx_df[['patientId', 'ART_TX']],
                         on='patientId').merge(tb_screen_df[['patientId', 'TB_screening']],
                                               on='patientId')
  # evaluate
  temp_df['TX_TB'] = ((temp_df['ART_TX'] == True) & (temp_df['TB_TX'] == True) &
                      (temp_df['TB_screening'] == True))
  temp_df = _gen_counts_and_ratio(temp_df, end_date, 'TX_TB')
  return temp_df

def calc_TX_ML(patient_agg_obs:  pd.DataFrame, ARV_plan: str,
                ARV_plan_answer: List[str], end_date_str: str = None) -> pd.DataFrame:
  """Calculates TX_ML indicator with its corresponding disaggregations.

  TX_ML indicator counts the number of  ART patients (who were on ART at the
  beginning of the quarterly reporting period) and then had no clinical
  contact since their last expected contact
  ...
  art_tx_df = patient_agg_obs[(patient_agg_obs['code'] == ARV_plan)].copy()
  art_tx_df['ART_TX'] = (art_tx_df['first_value_code'].isin(ARV_plan_answer))

  # check if patient is HIV +
  not_art_tx_df = patient_agg_obs[(patient_agg_obs['code'] == ARV_plan)].copy()
  not_art_tx_df['NOT_ART_TX'] = (not_art_tx_df['last_value_code'].isnull())

  # join the 3 DF
  temp_df = art_tx_df.merge(not_art_tx_df[['patientId', 'NOT_ART_TX']], on='patientId')

  # evaluate
  temp_df['TX_ML'] = ((temp_df['ART_TX'] == True) & (temp_df['NOT_ART_TX'] == True))
  temp_df = _gen_counts_and_ratio(temp_df, end_date, 'TX_ML')
  return temp_df