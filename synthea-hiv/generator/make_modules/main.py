...
def create_submodules_list(
    csv_file: str,
    submodule_type: Union[
        Type[multi_answer_mod.MultiAnswerSubmodule],
        Type[one_answer_mod.OneAnswerSubmodule],
    ],
) -> List[
    Union[multi_answer_mod.MultiAnswerSubmodule, one_answer_mod.OneAnswerSubmodule]
]:
    ...
    for _, df in df.groupby("QUESTION FROM AMPATH"):
        if pd.notna(df.iloc[0]["CIEL EQUIVALENT"]):
            t = submodule_type(
                ...
                common.CodeDisplay(
                    df.iloc[0]["QUESTION FROM AMPATH"], df.iloc[0]["CIEL EQUIVALENT"]
                ),
            )
            ...
            for _, row in df.iterrows():
                if pd.notna(row["CIEL CODE ANSWER"]):
                    t.add_answer(
                        common.CodeDisplay(
                            row["CODED ANSWERS FROM OPENMRS"], row["CIEL CODE ANSWER"]
                        )
                    )
    return submod_list
...
if __name__ == "__main__":
    submodules_list = create_submodules_list(
        "ampath-q-a.csv", one_answer_mod.OneAnswerSubmodule
    )
