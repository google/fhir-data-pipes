"""Uploads FHIR Bundle transactions to a FHIR endpoint.
...
parser.add_argument(
    '--convert_to_openmrs',
    action='store_true',
    help='specify if uploading to OpenMRS')
...
def list_all_files(directory: str) -> Dict[str, Set[pathlib.PosixPath]]:
  ...
  Args:
    directory: Directory containg JSON files

  Returns:
   Dictionary listing JSON files for hospitals, pracitioners, and patient
   history
  """
...
def convert_to_bundle(
    json_file: pathlib.PosixPath) -> bundle.Bundle:
  """Loops through list of JSON files, and loads content of each file to create Bundle object.

  Args:
   json_file_set: Set containing JSON files that need to be read
...
def upload_openmrs(sink: fhir_hook.BaseHook,
                   patient_bundle: bundle.Bundle):
  """Upload Patient history bundles to OpenMRS.

  For each bundle, we have to know the indiviual Patient, Encounters, and
  ...
  Args:
    ...
    list_of_patients_bundles: list of all the Bundles that need to be uploaded
  """
  ...
  # TODO(omarismail): implement save mapping
  patient_bundle.save_mapping()


def upload(sink: fhir_hook.BaseHook, each_bundle: bundle.Bundle):
  ...
  Args:
    ...
    file_dict: Dictionary listing JSON files for hospitals, pracitioners, and
      patient history
  """
...
def create_sink(url: str) -> fhir_hook.BaseHook:
  if 'https://healthcare.googleapis.com' in url:
    return fhir_hook.GcpHook(url)
  else:
    return fhir_hook.OpenMRSHook(url)
...
if __name__ == '__main__':
  ...
  if args.convert_to_openmrs:
    with multiprocessing.Pool() as p:
      ...
      bundle_list = p.map(convert_to_bundle, json_file_dict['patient_history'])      

      p.starmap(upload_openmrs, zip(itertools.repeat(fhir_sink), bundle_list))
    ...
    # hospital, practitioner, patient history
      for file_type in ['hospital', 'practitioner', 'patient_history']:
        with multiprocessing.Pool() as p:
          ...
          bundle_list = p.map(convert_to_bundle, json_file_dict[file_type])

          p.starmap(upload, zip(itertools.repeat(fhir_sink), bundle_list))
