...
If you are looking for a quick evaluation or prefer to deploy via docker image, check out the [single machine deployment section](link_here) which uses a pre-configured docker image and instructions for setting up an end-to-end deployment.
...
To learn how the Pipelines Controller works on its own, go to the FHIR FHIR Pipelines Controller.
