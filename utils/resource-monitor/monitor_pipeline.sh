...
cd .. 
cd ..
java -cp pipelines/batch/target/batch-bundled.jar com.google.fhir.analytics.FhirEtl --fhirServerUrl=http://localhost:8098/fhir --fhirServerUserName=hapi --fhirServerPassword=hapi --outputParquetPath=$3 --resourceList=Patient,Encounter,Observation --batchSize=20 --targetParallelism=$1 2>&1 &
